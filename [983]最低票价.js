...
var mincostTickets = function (days, cost) {
    //dp[i]表示在第i天完成前面旅游所需要的最低票价
    //dp[i]  = min(dp[i-1]+cost[0], dp[Math.max(i-7, 0)]+cost[1], dp[Math.max(i-30, 0)]+cost[2])
    let n = days[days.length - 1]
    let dp = new Array(n + 1).fill(0)
    for (let day of days) dp[day] = -1
    dp[0] = 0
    for (let i = 1; i <= n; i++) {
        if (dp[i] == 0) {
            dp[i] = dp[i - 1]
        } else {
            dp[i]  = Math.min(dp[i-1]+cost[0], dp[Math.max(i-7, 0)]+cost[1], dp[Math.max(i-30, 0)]+cost[2])
        }
    }
